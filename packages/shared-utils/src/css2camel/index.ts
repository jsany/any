...
export type TCss2camel = (str: string, opt?: IOpt) => { [prop: string]: string | number };
/**
 ...
 * @returns obj - 返回驼峰对象
 */
export const css2camel: TCss2camel = (str, opt = {}) => {
  const obj = {};
  ...
  arr1.forEach((item) => {
    ...
    // 将属性转为驼峰
    const key = arr2[0].replace(/-(\w)/g, (_k, r) => r.toUpperCase());
    let value = arr2[1];
    if (opt.rem) {
      const reg = /\b(\d+(\.\d+)?)PX\b/gi;
      // 先test下有没有符合的如果有再进行替换
      if (reg.test(value)) {
        value.replace(reg, (_k, r) => {
          let val = r / opt.unit!;
          // 精确到几位
